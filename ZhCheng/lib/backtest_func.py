...
def backtest(date_range, signal, buy_commission=2e-4, sell_commission=2e-4, *args):
    tax_ratio = 0.001                                   # 印花税
    ...
    for day in date_range:
        ...
        buy_shares = buy_values / AStocks_buy_price[buy_values.index]
...
def signal_to_effectivelist(day, signal, effective_number, transform_mode):
    ...
    effective_list = effective_list.sort_values(ascending=False)[:effective_number]
    ...
    if transform_mode == 1:
        ...
        normal = effective_list[(effective_list >= left_3std) & (effective_list <= right_3std)]
...
def effectivelist_to_targetlist(day, effective_list, last_hold_list, target_number):
    if not effective_list.empty:
        price = close_price_none.loc[day]
        last_price = last_close_price_none.loc[day]
        buy_limited_AStocks = price[price > 1.095*last_price].index
        sell_limited_AStocks = price[price < 0.905*last_price].index
        AStocks_opened = (stocks_opened.loc[day] == 1)
        opened_AStocks = AStocks_opened[AStocks_opened].index
        
        coincident_list = last_hold_list[last_hold_list.index.isin(effective_list.index)]
        selling_stocks = set(last_hold_list.index) - set(coincident_list.index) - set(buy_limited_AStocks)
        ...
        if len(hold_list) < target_number:
            buyable_stocks = ((set(effective_list.index) & set(opened_AStocks)) - 
                               set(buy_limited_AStocks) - set(sell_limited_AStocks))
            effective_list = effective_list[list(buyable_stocks)]        
            alternative_list = effective_list.drop(hold_list.index, errors='ignore')
...
def signal_to_targetlist(day, signal, last_hold_list, effective_number, target_number, transform_mode):
    if day in signal.index:
        effective_list = signal_to_effectivelist(day, signal, effective_number, transform_mode)
        target_list = effectivelist_to_targetlist(day, effective_list, last_hold_list, target_number)
    else:
        target_list = last_hold_list
