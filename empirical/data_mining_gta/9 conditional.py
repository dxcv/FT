...
# TIME:2018-09-19  09:08
# NAME:FT_hp-9 conditional.py
...
DIR_COND=os.path.join(DIR_DM_GTA,'analyse','conditional')

#conditional on idiosyncratical volatility
def get_indicator_direction(indname):
    at = pd.concat(
        [pd.read_pickle(os.path.join(DIR_DM_GTA,'analyse', f'at_{bench}.pkl'))
         for bench in BENCHS], axis=1,sort=True,keys=BENCHS)
...
def get_comb_indicators(critic=2):
    ...
    if os.path.exists(path):
        ...
        indicators = pd.concat([pd.read_pickle(
            os.path.join(DIR_DM_GTA,'normalized',
...
def get_comb(cond_variable):
    ...
    comb=comb.groupby('month_end').filter(lambda df:df.shape[0]>CROSS_LEAST)#trick: filter out months with too small sample
    comb=comb.fillna(0)
