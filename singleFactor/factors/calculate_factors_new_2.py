...
def get_dividend3YR():
    ...
    df=x_history_compound_growth(df, 'cash_div', q=12, ttm=False)
    check_raw_level(df,'target',name)
...
def get_operCashInToAsset():
    ...
    df=get_dataspace(['net_cash_flows_oper_act','tot_assets'])
...
def get_roa():
    ...
    name='roa'
    df=get_dataspace(['net_profit_incl_min_int_inc','tot_assets'])
...
def get_netProfitRatio():
    ...
    df['target']=df['net_profit_incl_min_int_inc']/df['tot_oper_rev']
    check_raw_level(df,'target',name)
