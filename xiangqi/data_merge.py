...
def factor_merge(df1, df2):
    ...
    monthly_data = data.groupby('stkcd').resample('M', on='trd_dt').last() # resample 得到的是calendar date,mont_end
    monthly_data=monthly_data.reset_index(drop=True).set_index(['stkcd','trd_dt'])

#    data = data.set_index(['stkcd', 'trd_dt'], drop=False)
#     data = data[(data.type_st == 0) & (data.year_1 == 0)]
    data=data[(~data['type_st']) & (~ data['young_1year'])]
    #TODO： 涨跌停剔除，
