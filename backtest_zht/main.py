...
def signal_to_effectivelist(day, signal):
    ...
    effective_list = effective_list.sort_values(ascending=False)[:CONFIG['effective_number']] # keep the largest n (effective_number) stock
...
def effectivelist_to_targetlist(day, effective_list, last_hold_list):
    if not effective_list.empty:
        ...
        last_price = last_close_price_none.loc[day] #trick: we had to use the non adjusted price
        ...
        if len(hold_list) < CONFIG['target_number']:
            ...
            complement_number = CONFIG['target_number'] - len(hold_list)#这种定量持有100股的持仓方式可以优化一下，不一定非要持有100股
...
def signal_to_targetlist(day, signal, last_hold_list):
    if day in signal.index:
        effective_list = signal_to_effectivelist(day, signal)
...
def backtest(date_range, signal):
    ...
    for day in date_range: #在策略第一天运行的时候肯定不能直接这样在第一天建仓100只股票，会造成市场冲击，所以，策略在高换手率期间应该有所优化
        ...
        sell_fee = sell_values_sum * (CONFIG['tax_ratio']+CONFIG['sell_commission'])


        if last_hold_shares.empty:
            # buy_values_sum=capital/(1+buy_commission)
            buy_values_sum=CONFIG['capital']/(1+CONFIG['buy_commission'])
        else:
            buy_values_sum=(sell_values_sum-sell_fee)/(1+CONFIG['buy_commission'])
...
def quick(signal,fig_title,start=None, end=None):
    ...
    perf_yearly=format_year_performance(trade_returns,benchmark,turnover_rates)
    hedged_returns = get_hedged_returns(trade_returns, benchmark)
