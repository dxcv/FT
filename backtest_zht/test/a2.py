...
class Backtest:
    ...
    def signal_to_effectivelist(self,day, signal):
        ...
        为了方便，我们这里直接省略了这个步骤，直接用股票代码排序，s.sort_index().sort_values(ascending=False,kind='mergesort')[:n]以保证回测结果的唯一性。但是，这样会导致每次遇到这种
        情况，我们都会优先选择股票代码较小的股票。（后边所有用Series排序选股票的方式都有这个问题）
        注意，sort_values() 默认使用的是quicksort.This is NOT a "stable" algorithm, 
        in that it does not preserve "the input order of equal elements in the 
...
name='a2'
Backtest(signal,'a2',os.path.join(DIR_TMP,'a2'))
