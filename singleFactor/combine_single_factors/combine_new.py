...
def get_strategy_ret():
    path=os.path.join(DIR_TMP,'dalsfjdaslf.pkl')
...
def _grade(ret, rating_func,freq,window):
    ...
    # mss=multi_process(_apply_rating_func, args_list) #fixme:
    mss=[]
...
def get_month_data(grade,rb_dt,criteria,N):
    ...
    d=pd.Series(r,index=s.values).to_dict()
