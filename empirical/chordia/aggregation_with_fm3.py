...
# TIME:2018-09-05  11:19
# NAME:FT_hp-aggregation_with_fm3.py
import pandas as pd
import os
import numpy as np
...
def pricing_with_fm_augmented(indicator_number):
    ...
    indicators=pd.concat([pd.read_pickle(os.path.join(DIR_DM_NORMALIZED, sl + '.pkl')).stack() for sl in selected], axis=1, keys=selected)
