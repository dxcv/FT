...
# TIME:2018-09-18  22:49
# NAME:FT_hp-7 bootstrap.py
import pandas as pd
from empirical.config_ep import DIR_DM_GTA
from empirical.data_mining_gta.dm_api import get_data
...
DIR_BOOT=os.path.join(DIR_DM_GTA,'analyse','bootstrap')


def get_realized(bench_name):
    path=os.path.join(DIR_BOOT, f'realized_{bench_name}.pkl')
...
def simulate(bench_name, n):
    path=os.path.join(DIR_BOOT, f'{bench_name}_{n}.pkl')
    if os.path.exists(path):
        return
    ...
    rs=multi_process(bootstrap_yan, ((benchmark,assets,realized) for i in range(n)),20,
                     multi_parameters=True,size_in_each_group=200) #review:
...
def get_bootstrap_pvalue(bench_name, n=1000):
    ...
    simulated=pickle.load(open(os.path.join(DIR_BOOT, f'{bench_name}_{n}.pkl'), 'rb'))
    realized=get_realized(bench_name)
    ...
    df.to_csv(os.path.join(DIR_BOOT, f'result_{bench_name}.csv'))

def bootstrap_t_pvalue(bench_name):
    # benchmark, assets = get_data(bench_name)
    simulated=pickle.load(open(os.path.join(DIR_BOOT, f'{bench_name}_1000.pkl'), 'rb'))
    ...
    b_p.to_pickle(os.path.join(DIR_BOOT,f'{bench_name}_bp.pkl'))
...
#TODO:ｂｏｏｔｓｔｒａｐ　ｆｏｒ　ｆａｍａ　ｍａｃｂｅｔｈ
